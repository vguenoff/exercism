...
describe('createVisitor', () => {
    test('correctly assembles the visitor object', () => {
        ...
        expect(createVisitor('Nālani Sansone', 76, 'YX52GB06')).toEqual(
            expected2,
        )
...
describe('ticketStatus', () => {
    test('correctly identifies that a ticket is not in the tracking object', () => {
        expect(ticketStatus(testTickets(), 'Y4KXZOYM')).toBe(
            'unknown ticket id',
        )
        expect(ticketStatus(testTickets(), '8ATQC1ZJ')).toBe(
            'unknown ticket id',
        )
        expect(ticketStatus(testTickets(), 'G833HR8A')).toBe(
            'unknown ticket id',
        )
    })
...
describe('simpleTicketStatus', () => {
    ...
    test('returns the visitor name for tickets that were sold', () => {
        ...
        expect(simpleTicketStatus(testTickets(), 'H31SAW5Q')).toBe(
            'Lior MacNeil',
        )
        expect(simpleTicketStatus(testTickets(), 'M9ZTXP89')).toBe(
            'Kamani Ybarra',
        )
    })
